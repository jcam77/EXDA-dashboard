...
const UnifiedModal = ({ modal, setModal }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-card p-6 rounded-xl border border-border w-[450px] shadow-2xl flex flex-col gap-4 relative text-foreground">
                <button onClick={() => setModal({...modal, show: false})} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground"><X size={20}/></button>
                ...
                <div className="flex justify-end mt-2 gap-2">
                    {actions ? (
                        ...
                        <button onClick={() => setModal({...modal, show: false})} className="px-6 py-2 bg-muted hover:bg-muted/80 text-foreground rounded-md text-sm font-bold border border-border">Close</button>
                    )}
                </div>
            </div>
        </div>
